...
titanic = as.matrix(read.table("titanic.dat", header=FALSE));
n = dim(titanic)[1]
socialClass = matrix(rep(0, n*3), nrow = n, ncol=3);
for(j in 1:3){
	socialClass[titanic[, 1] == j, j] = 1;
	}

xTrain = cbind(socialClass, titanic[, 2], titanic[, 3]);
yTrain = titanic[, dim(titanic)[2]]
...
# (DIC).
count = 0;
D = rep(0, length(burnIn:nIterations))
for(i in burnIn:nIterations){
    count = count+1;
    eta = xTrain%*%beta[, i];
    theta = exp(eta)/(1+exp(eta));
    D[count] = -2*sum(log(dbinom(yTrain, ni, theta)));
}
 
D_avg = mean(D)
 
betaHat = apply(beta[, seq(burnIn, nIterations)], 1, mean);
eta = xTrain%*%betaHat;
thetaHat = exp(eta)/(1+exp(eta));
D_thetaHat = -2*sum(log(dbinom(yTrain, ni, thetaHat)))
 
p_D = D_avg - D_thetaHat
DIC = 2*D_avg - D_thetaHat


return(list(beta = beta, D_avg = D_avg, p_D = p_D, DIC = DIC))
}
...
# variable z = log(y) = g(x_) -e, where e is exponentially distributed with
