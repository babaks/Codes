...
% To run the code type [mu_true, sigma2_true, mu0_true, mu, sigma2, mu0] =
% modelHousePrice;
function [mu_true, sigma2_true, mu0_true, mu, sigma2, mu0] = modelHousePrice
...
mu_true = normrnd(mu0_true, 4, 1, J);
...
% Withing each state "j", I sample 100 values from N(mu_j, 5.35).
% The first column of variable "house" holds the state identifier. 
house = [];
for i = 1:J
    house = [house; [repmat(i, nj, 1), normrnd(mu_true(i), sqrt(sigma2_true), nj, 1)]];
end
...
% y now includes the observed variables.
y = house(:, 2);
...
for i = 2:nIterations
   ...
   % This is (conditionally) conjugate situation, so I can directly sample
    ...
    mu(i, j) = normrnd(mu_n, sqrt(sigma2_n));
    yCent = [yCent; yObs - mu(i, j)];
    end
    ...
    sigma2_n = 1/(1/V + n/tau02);
    mu0(i) = normrnd(mu_n, sqrt(sigma2_n));
