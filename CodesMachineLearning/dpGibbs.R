...
# We generate data from two normal distributions: 20 samples with means 0 and 10 samples with 5; we fix the variance at 1
set.seed(123)
...
# Baeline distribution G0 is assumed to be N(0, 100)
m = 0
tau2 <- 100
...
for(j in 1:nIter){
  for(i in 1:n){
    ...
    q0 <- gamma*dnorm(x[i], m, sqrt(sigma2+tau2)) 
    ...
    Thetas <- c( rnorm(1, (m/tau2+x[i]/sigma2)/(1/tau2+1/sigma2), sqrt( 1/(1/tau2+1/sigma2) ) ), theta[-i, j]) 
...
# We throw away the first 3000 samples and take average using the remaining samples
...
p <- ggplot(simData.df, aes(x=x, group=y)) + geom_density() 
p <- p + geom_point(aes(x=x, y=y), cex=2, data=origData.df)
print(p)
dev.off()
